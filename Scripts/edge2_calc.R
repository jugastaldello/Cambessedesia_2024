rm(list=ls())
## Calculating GE2 and EDGE2 for Cambessedesia species
# October 2024
library(ape)

setwd("~/Desktop/camb_edge2")
...
#Getting the tree
tree_c <- read.tree("tree_final.txt")
tree_c$tip.label <- gsub(pattern = "_", replacement = " ", 
                       x = tree_c$tip.label)
...
tree_c$tip.label <- gsub(pattern = " ", replacement = "_", x =tree_c$tip.label)
...
write.csv(final_plot_data, "edge2_max_min_values.csv") #saving the table for max and min values
...
## Blank plot
plot(1, type="n", xlab="sp", ylab="EDGE2 range", xlim=range(x), 
...
write.csv(final_plot_data2, "edge2_mean_values.csv") #saving the data for mean values


barplot(final_plot_data2$Mean)
barplot(rev(final_plot_data2$Mean), horiz=TRUE, width = 1, space=0,
        ylim=c(1,length(tree_c$tip.label))-0.75, xlim = c(0, 3), names="", col="azure3")
